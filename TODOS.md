...
- [ ] Phase 16: split `production-audit` into per-dimension + rubric reference files (215 lines / 4,391 words all loaded at invocation; the fan-out should hand each subagent its own dimension file). Deferred out of v0.1.1 by its scope law because it creates new artifacts; must ship inside `skills/production-audit/` on the bundled-schema pattern. Write its plan file when Workstream J closes.
