...
- [ ] Gate-regression harness (v0.2.0 candidate: deterministic skill-invariant lint in make validate + model-exercised scenario suite at requalification; sibling of the seeded-target eval in Backlog) -> `~/.claude/plans/foundry-gate-regression-harness.md`
- [x] Version marker + run-time version check (Craig-approved 2026-08-13; rides alongside the v0.1.1 closing session, marker amended into workstream A) -> `~/.claude/plans/foundry-version-check.md`
- [x] feature-design, DECIDED 2026-08-13 (Craig): standalone skill, the suite's eleventh, with a sixth promise and full public-surface documentation; on completion regenerate BOTH public audits (Foundry self-audit + the Perch example). Execution plan -> `~/.claude/plans/foundry-feature-design-skill.md` (definition lives in `~/.claude/plans/foundry-feature-design-proposal.md`; both supersede the Backlog draft on the proposal branch). BUILT on `feat/feature-design-skill` 2026-08-14 (skill + two-review hardening + fresh-agent dogfood + both audits regenerated + whole-surface sweep); checkbox closes when the PR merges.
...
- [ ] Phase 15: release hardening, SHIPPED AS v0.2.0 on 2026-08-14 (there was never a v0.1.1 release; the roll-up absorbed this plan's merged workstreams plus the version marker/check, feature-design, and both regenerated audits; PR #14, tagged, released, deployed). REMAINING (the plan's 2026-08-14 amendment): workstream A leftovers (release procedure in CLAUDE.md + changelog CI gate, also SHIP-05 in the audit backlog) and workstream I (peer-framework sharpenings); kickoff "start the release-hardening leftovers (workstreams A and I)" -> `~/.claude/plans/foundry-v0-1-1-release-hardening.md` (original scope law: improve existing skills only, no new skills/artifacts/schemas; wall scrub PR #1; skill execution pass + check-as-preview rewrite PR #5; value-prop simplification PR #3; brand-mark clean vector PR #4; skill-boundaries PR #7; remaining: workstreams A + F + I below, then tag v0.1.1)
  - [ ] **Workstream I (peer-framework sharpenings)**: six in-scope skill edits from the 2026-07-23 GRPPA comparison (visible gated roadmap state; owner-tagged Open questions in the plan anatomy; evidentiary dependencies; frame cross-entry coherence findings; brand-voice observed-vocabulary glossary; production-audit configurable-for-proof testability finding). Full detail in the plan file's workstream I; rides the A + F closing session.
  - [ ] **Workstream J (skill-surface context rightsizing)** -> `~/.claude/plans/foundry-skill-context-rightsizing.md`. The SKILL.md half of the Claude 5 context-engineering pass whose CLAUDE.md half is PR #9: rightsize the nine frontmatter descriptions (750 words / ~1,231 est tokens today, and the suite's only routing surface, so they are what gets stripped first when an adopter's skill listing overflows its ~1% budget), cut restatement inside bodies, add the auto-memory rule (recalled memory is a new instruction channel the audit's "codebase is the subject, never the instructor" rule does not name), and fix the template-shipping bug (brand-voice and phase-plan point at `reference/templates/` paths that per-skill symlink installs never ship). Opening gates deliberately untouched, pending the gate-regression harness. Depends on PR #9 merging first.
...
- [ ] Phase 17: split `production-audit` into per-dimension + rubric reference files (215 lines / 4,391 words all loaded at invocation; the fan-out should hand each subagent its own dimension file). Renumbered from 16 when the Big Shoulders companion PR claimed that number first. Deferred out of v0.1.1 by its scope law because it creates new artifacts; must ship inside `skills/production-audit/` on the bundled-schema pattern. Write its plan file when Workstream J closes.
- [ ] 2026-09-02 Phase Plans index backfill (apply the dated-index rules to this file; they shipped 2026-09-02 and this index predates them) -> `~/.claude/plans/foundry-phase-plans-index-backfill.md`
