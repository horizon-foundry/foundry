...
- [ ] Skill pressure-test learnings (external adversarial audit, 2026-08-27): the suite's skills were used as review INSTRUMENTS inside a deep multi-lens audit of a live product, and each invocation was meta-observed. Per-skill improvement candidates feeding v0.2.0: instrumentation needs a first-class audit mode (the identity-stitch check lives only on the build path); brand-voice needs a quote/citation-provenance step (its worst finding came from the reviewer, not the skill); mobile needs a "first viewport shows the flow's primary action" matrix row; frame needs a stamp-deferred audit mode and a "is the frame still true of the world" gate; and a shared audit-stance convention so build-oriented skills degrade gracefully into review instruments. Plus the genuine GAP: no skill covers adversarial data-model or security review (the two most severe findings came from hand-run passes), arguing a data-layer-audit and/or security-review lens with a sibling-call-site sweep -> `~/.claude/plans/foundry-savor-pressure-test-learnings.md`
