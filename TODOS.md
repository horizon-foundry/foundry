...
- [ ] feature-design, DECIDED 2026-08-13 (Craig): standalone skill, the suite's tenth, with a sixth promise and full public-surface documentation in the readout PR's pattern; on completion regenerate BOTH public audits (Foundry self-audit + the Perch example) under the then-current method. Sequenced after the v0.1.1 closing session and the version-marker PR. Proposal + drafted definition -> `~/.claude/plans/foundry-feature-design-proposal.md` (supersedes the Backlog draft on the proposal branch); execution plan written at the version-marker PR's close.
