...
- [ ] Phase 15: release hardening, SHIPPED AS v0.2.0 on 2026-08-14 (there was never a v0.1.1 release; the roll-up absorbed this plan's merged workstreams plus the version marker/check, feature-design, and both regenerated audits; PR #14, tagged, released, deployed). CLOSED 2026-08-14 on `chore/release-hardening-leftovers`: workstream A leftovers (release procedure in CLAUDE.md + the changelog CI gate, `scripts/check-changelog.mjs`, both failure and waiver paths exercised; SHIP-05 in the audit backlog) and workstream I (six peer-framework sharpenings). Only the homepage grid-background design-hook flag stays open, and it is Craig's call, not work -> `~/.claude/plans/foundry-v0-1-1-release-hardening.md` (original scope law: improve existing skills only, no new skills/artifacts/schemas; wall scrub PR #1; skill execution pass + check-as-preview rewrite PR #5; value-prop simplification PR #3; brand-mark clean vector PR #4; skill-boundaries PR #7; remaining: workstreams A + F + I below, then tag v0.1.1)
  - [ ] **Workstream I (peer-framework sharpenings)**: six in-scope skill edits from the 2026-07-23 GRPPA comparison (visible gated roadmap state; owner-tagged Open questions in the plan anatomy; evidentiary dependencies; frame cross-entry coherence findings; brand-voice observed-vocabulary glossary; production-audit configurable-for-proof testability finding). BUILT with workstream A on `chore/release-hardening-leftovers` 2026-08-14 (all six landed; two judgment calls recorded in NOTES: the gated marker lives in phase-plan alone per one-owner-per-fact, and Foundry's own approved glossary rows keep no back-filled provenance). Checkbox closes when the PR merges.
  - [ ] **Workstream J (skill-surface context rightsizing)** -> `~/.claude/plans/foundry-skill-context-rightsizing.md`. The SKILL.md half of the Claude 5 context-engineering pass whose CLAUDE.md half is PR #9: rightsize the frontmatter descriptions (the suite's only routing surface, so they are what gets stripped first when an adopter's skill listing overflows its ~1% budget) and cut restatement inside bodies. Opening gates deliberately untouched, pending the gate-regression harness. RECONCILED 2026-08-14 (plan file amended in the same action): the auto-memory rule and the template-shipping fix are already DONE, PR #9's dependency has cleared, and the measurement is now eleven skills at 616 words / ~1,010 est tokens (was nine at 750 / ~1,231), so the remaining cut is smaller than the plan assumed. This is the next unit; kickoff "start workstream J (skill-surface context rightsizing)".
