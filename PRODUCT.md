...
- **readout** reviews the surfaces that display the numbers instrumentation captures: pitched at a named reader with the detail one click down, a window and a population on every face, a failed read that never renders as a zero, and the rendered result reviewed by someone who did not build it.
