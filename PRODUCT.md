...
The suite makes six promises, each kept by a skill:

1. **Product intent is declared and audited** before building (frame).
2. **A feature's design is checked against the code before it is built** (feature-design).
3. **Execution context survives every session** (phase-plan and the plan chain).
4. **The intended user outcome is instrumented** (instrumentation).
5. **The documentation matches reality** (document).
6. **Technical readiness is audited before real users arrive** (foundry + production-audit).
...
- Entry 9 is a blank: no learn-before-investing threshold was declared before the build grew to nine skills and a site. <!-- skills-count-exempt: dated finding, the count was nine when it was written -->
...
- **frame** audits the builder's declared one-page product frame before anything gets built: user, problem, alternatives, outcomes, evidence and assumptions, success measure, non-goals, risks, learn before investing, plus a four-line security frame (data classification, trust boundaries, authn/authz, secret handling). Blank or vague entries are findings, like the audit's empty matrix cells, and so are two entries that contradict each other: a page can disagree with itself while every line passes on its own. Nothing enters the frame that the builder did not state or confirm. The frame opens PRODUCT.md, and the rest of the work is checked against it. Discovery and structured elicitation are other tools' work. frame evaluates; it does not extract.
- **feature-design** audits a declared feature design against the actual repository, before any implementation plan is written. The builder declares six entries (behavior, surface area, decisions with the alternative each beat, non-goals, verification, scope); the skill grounds every proposed behavior and named path in the code and leaves a grounding table (absent, partial, already present, present but unreachable, unrequested), with file and line on every code-grounded finding. It never interviews toward a design, writes no plan, and issues no verdict; its best finding is the feature that turns out to be a reachability fix.
...
- **readout** reviews the surfaces that display those measurements: a dashboard, a report, an experiment read-out, a metrics email. Instrumentation makes the numbers exist; readout makes them worth reading. It pitches each surface at a named reader and the decision they make, keeps the supporting detail one elevation down instead of in the way, puts a window and a population on every face so a rate cannot be read as a count, and keeps a failed read, a genuine zero, and a stale cache visibly distinct rather than collapsing all three into "0". It ends where these defects actually surface: in the rendered pixels, reviewed by someone who did not build them.
- **phase-plan** and **brand-voice** cover two disciplines. phase-plan keeps the plan chain unbroken across sessions (or honestly terminal: decision required, awaiting evidence, or a deliberate stop), and a phase that cannot start yet says what it waits on at the index rather than looking like one nobody has picked up. brand-voice keeps one voice from `BRAND.md` across every surface, a voice that binds only after human sign-off, never outranks clarity, and takes its vocabulary from what the builder already says rather than inventing a language at branding time. (The reading half of the plan chain, read the indexed plan and reconcile it against git state before acting on a terse continuation, is a standing instruction that a scaffolded `CLAUDE.md` carries, not a separate skill.)
