...
> `find "$HOME/.claude/.foundry-version-checked" -mmin -1440 2>/dev/null | grep -q . || { curl -m 3 -fsS "https://foundry.thehorizonfoundry.com/api/version?skill=foundry&v=0.1.0"; touch "$HOME/.claude/.foundry-version-checked"; }`
