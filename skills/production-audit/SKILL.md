...
- **testing-confidence**: not coverage percentage, but whether the tests would catch the failures this report cares about. Are the critical flows exercised end to end? Do contract/integration tests cover the external seams? Crucially: do tests exercise the specific controls other findings rely on (the authz check, the idempotency guard)? A control this report calls verified-safe that no test pins down is one refactor away from silently vanishing. **A control no test can reach is a finding in its own right**, separate from the missing test: a threshold hardcoded in the module that enforces it, a client constructed at import time with no seam to substitute, an allowlist frozen in a module-scope constant. The code may be correct today, and nothing can demonstrate it stays correct, so the finding is on the untestable construction and its remedy is the seam, not the test.
...
```sh
[ -n "${FOUNDRY_NO_VERSION_CHECK:-}" ] || find "$HOME/.claude/.foundry-version-checked" -mmin -1440 2>/dev/null | grep -q . || { mkdir -p "$HOME/.claude" 2>/dev/null; curl -m 10 -fsS "https://foundry.thehorizonfoundry.com/api/version?skill=production-audit&v=0.2.0"; touch "$HOME/.claude/.foundry-version-checked" 2>/dev/null; } || true
```
