...
> **Using this skill:** announce "Using production-audit", make a todo per step below, and do not skip the gates. This skill's worth is its process, not a hand-reproduced outcome. If you were told to "run production-audit", run it, do not improvise the result. (Suite standard: https://github.com/horizon-foundry/foundry/blob/main/reference/skill-authoring.md)
...
**Every finding carries a `kind`: risk or improvement.** A **risk** has a path to harm in production (exposure, data or payment corruption, a crash or unavailability, abuse, or a hidden active failure). An **improvement** is safe today but makes the system more robust, observable, consistent, accessible, or faster. The distinction is load-bearing: **only risks drive the verdict; an improvement never caps it.** A report with no risks is clear within its assessed scope even if it lists a dozen improvements, because a punch list of betterments is not a reason to hold a release. This is also what keeps the report honest and readable: risks to weigh first, improvements to schedule second, never one undifferentiated wall of problems. A quality tool must be able to say "this is good, here is how to make it better" without that reading as "this is broken." Think of it as bugs versus tech debt. Severity and kind are related but distinct: **a finding minor enough to rate low or informational is an improvement (tech debt or polish), not a ship risk** (a low rate-limit gap already mitigated upstream, a rare recoverable race, a consistency divergence, an accessibility refinement). Risks are the ship-relevant harm paths, which in practice sit at medium and above. And a medium-or-higher item with no harm path today, missing tests, missing CI, observability thinness, is an improvement too. What earns "risk" is a live exposure, a corruption path, a crash, or a silent availability failure the shipper must consciously accept.
...
1. **Flow inventory.** Every privileged or mutating flow, end to end: signup/login, invites, uploads, payments, cron jobs, webhooks, LLM calls, deletes. For each: entry point, auth boundary crossed, data written, side effects.
2. **Trust-boundary map.** Where untrusted input enters (forms, URLs, file uploads, webhooks, LLM output) and where privilege changes (middleware, layouts, DB policies, service-role clients).
...
One read-only subagent per dimension, in parallel, each given the Phase 0 artifacts. Every finding must cite file and line. If the harness cannot spawn parallel sub-agents (or requires a permission the run does not have), do not skip dimensions: run them sequentially as focused passes in the main context, or stop and ask the user to enable parallel work. Coverage is what matters; the fan-out is only a way to reach it faster. Required artifacts per dimension:
...
- **release-safety**: how a bad version reaches users and how it gets pulled back. Rollback path and its speed, feature flags or staged rollout for risky changes, forward/backward compatibility between the deployed version and its data/clients, and whether deploy and release are separable.
...
**Pre-ship gates: mobile and instrumentation.** These are release gates, not ordinary dimensions: the audit always checks them and reports where each stands. How a gate failure weighs the verdict follows the project's declared release policy (see `foundry`): a **required** gate's failure caps the verdict, an **optional** gate's failure is reported but does not by itself cap it, and a **waived** gate is recorded with its reason (for instrumentation, a documented no-telemetry or trust-boundary posture is a legitimate waiver). Absent a declared policy, default both to required for any real audience. A gate is never silently omitted, whatever its disposition.

- **Mobile** (apply the `mobile` skill's discipline). The product must hold up on a real device: zero horizontal overflow at a real device width, touch targets, the keyboard not occluding controls, touch vs pointer handled. A core flow that breaks on a phone fails the mobile ship gate, and a failed required gate blocks a broad launch. Keep the gate failure and the finding's severity separate: rate the finding on the rubric (a broken core flow is often High, but earn it on impact, reach, and recoverability), do not auto-assign High just because a gate failed.
- **Instrumentation** (apply the `instrumentation` skill's discipline). The product must be measurable. Verify the activation funnel is instrumented on one identity across client and server. An uninstrumented activation path is a ship-gate finding: where instrumentation is a required gate it caps the verdict at "ready to ship, risks noted" at best; where the policy makes it optional or waives it (a documented no-telemetry posture), that is recorded rather than capping.

**Report each gate's status.** Every report emits a `shipGates` entry per gate (`met`, `at-risk`, `not-met`, `not-assessed`, or `not-applicable`), with a note and any informing finding IDs. This is not optional: if the method calls mobile and instrumentation release gates, the artifact has to say where each one stands. A `not-met` required gate caps the verdict below safe-to-ship. On a static-only run, the device-level mobile checks and the live activation funnel are runtime work. Report what the code shows, mark the runtime portion `not-assessed` with the check to run, and never assert a gate you could not verify. A gate that does not apply (for example, mobile on a headless service) is `not-applicable` with a one-line reason.

### Phase 3: Adversarial verification

Every Critical, High, and **verdict-driving** finding goes to a fresh subagent whose only job is to **refute** it against the code: Is it mitigated at another layer? Fixed by a later migration? Covered by an existing test? Is the exploit path actually reachable? A verdict-driving finding is any finding cited in `verdict.blockingFindingIds`, whatever its severity: if it moves the verdict, it gets verified, because the verdict is only as trustworthy as the findings it rests on. Refuted findings are dropped or downgraded, with the refutation recorded in `verification.refutationNotes`. Other findings are verified when their uncertainty and impact warrant it, and are otherwise marked `not-verified`. Hard rule: the final verdict may not ship while any finding in `blockingFindingIds` still has `verification.status: not-verified` (the repo's `make validate` enforces this mechanically).
