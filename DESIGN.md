...
The **wordmark** pairs the mark with "Foundry", the suite. The parent's own HORIZON FOUNDRY lockup is the kit's, ships outlined, and is never re-typeset here or substituted into this site's chrome: what this site sets in live text is its own sub-brand lockup. Set it only in Archivo Black, never the mono or serif, uppercase, at **0.04em tracking**: the kit's +40, which is what the parent's own lockup and the kit's live-text wordmark class both use, and what the kit's rollout checklist asks each site to confirm. The two lockups are set identically and differ only in the word they carry (decided 2026-09-22; this site shipped 0.02em until then, and the claim that it matched the studio was simply false).
